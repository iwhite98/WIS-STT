...
def evaluate(
    preds: dict, gt_path: str, output_file: str, dataset: str = "scannet", rank: int = 0
):
    global CLASS_LABELS
    ...
    os.makedirs(f"{output_file}/{rank}")
    for i in range(4):
        with open(f"{output_file}/{rank}/ap{i}.pkl", 'wb') as f:
            print(f"{rank}, : dump ap{i}.pkl file")
            pickle.dump(matches, f)
    time.sleep(1)
    
    n_matches = {}
    for i in range(4):
        while not os.path.isfile(f"{output_file}/{i}/ap{rank}.pkl"):
            time.sleep(1)
        time.sleep(1)
        with open(f"{output_file}/{i}/ap{rank}.pkl", 'rb') as f:
            print(rank, ', load ', os.path.isfile(f"{output_file}/{i}/ap{rank}.pkl"), f"{output_file}/{i}/ap{rank}.pkl", )
            n_matches.update(pickle.load(f))
    ap_scores = evaluate_matches(n_matches)
    ...
    if rank == 0:
        ...
        write_result_file(avgs, f"{output_file}/tmp_output.txt")
    while not os.path.isfile(f"{output_file}/tmp_output.txt"):
        time.sleep(1)
    #print('#############')
    #print('!!!!!!!!!!', rank, len(list(n_matches.keys())))
    #print('#############')
    if dataset == "s3dis":
