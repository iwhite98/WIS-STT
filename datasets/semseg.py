...
class SemanticSegmentationDataset(Dataset):
    ...
    def __init__(
        ...
        if self.cache_data:
            ...
            # self._data = new_data
        #if mode == 'train':
        #    self._data = self.data[:10]
        #else:
        #    self._data = self.data[:62]
        #print('################', mode, len(self.data)) ##1201
        #exit()
    ...
    def __getitem__(self, idx: int):
        ...
        if self.dataset_name == "stpls3d":
            ...
            train_mode = False
            #print(self.data[idx]["raw_filepath"].split("/")[-2])
            #exit()
            if "train" in self.data[idx]["filepath"]:
                train_mode = True
                '''
                file_i = self.data[idx]["filepath"].split("/")[-1].split(".")[0]
                ps_path = "data_gapro/gaussian_process_kl_pseudo_labels/" + "scene" + file_i + ".pth"

                ps_semantic_label, ps_instance_label, ps_prob_label, ps_mu_label, ps_variance_label = torch.load(ps_path)
                
                ps_prob_label = torch.tensor(ps_prob_label)

                ps_path2 = "~/yjyoo_seg/GaPro/test/" + "scene" + file_i + ".pth"
                ps_semantic_label, ps_instance_label, ps_prob_label2, ps_mu_label, ps_variance_label = torch.load(ps_path2)
                ps_prob_label2 = torch.tensor(ps_prob_label2)
                print((ps_prob_label == 1.0).float().sum(), (ps_prob_label2 == 1.0).float().sum())
                exit()
                '''
                #ps_semantic_label = torch.tensor(ps_instance_label)
                

                #labels[:, 0] = ps_semantic_label
                #labels[:, 1] = torch.tensor(ps_instance_label)## 100 * 128
