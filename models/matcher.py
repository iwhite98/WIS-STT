...
class HungarianMatcher(nn.Module):
    ...
    @torch.no_grad()
    def memory_efficient_forward(self, outputs, targets, mask_type, is_eval = False, is_s = True):
        ...
        for b in range(bs):
            ...
            if not is_eval:
                tgt_mask = tgt_mask[:, targets[b]["segment_det_idx"]]
                out_mask = out_mask[:, targets[b]["segment_det_idx"]]
            ...
            '''
            query_mask = targets[b]["segment_mask"].clone()
            query_mask = query_mask[:, outputs["fps_idx"][b][:100]].cpu()
            C = torch.where(query_mask.T == 1, C, 1e8)
            values = torch.topk(
                C, 1, dim=0, sorted=True,
                largest=False).values[-1:, :]
            indices.append(torch.argwhere(torch.logical_and(C <= values, C < 1e8)))
            '''
        return [
            (
    ...
    @torch.no_grad()
    def forward(self, outputs, targets, mask_type, is_eval = False, is_s = True):
        ...
        Returns:
            ...
            For each batch element, it holds:
                len(index_i) = len(index_j) = min(num_queries,cd num_target_boxes)
        """

        return self.memory_efficient_forward(outputs, targets, mask_type, is_eval, is_s)
